...
class VIC_constant(nn.Module):
    def __init__(self, attr = 'size'):
        ...
        self.name = 'attr_decoder_mask_oig_' + '_' + self.attr
    ...
    def decode_attr(self, x):
        ...
        kv = torch.Tensor([[11,12,12,12,12,12]]).repeat(b, 1).to(device).long()
        # print(kv)
    ...
    def decode_attr_with_position(self, x, position = None):
        ...
        for i in range(1,6):
            ...
            if position is not None:
                ...
                mask = torch.cat([torch.zeros(1,1,self.attribute - 1), torch.ones(1,1,1)], dim = -1).to(x.device)
                
                position_now = position[:,None,i-1:i] 
                
                x_attr.masked_fill_((mask*position_now.to(mask.device))== 1, -1e9)
...
if __name__ == '__main__':
    ...
    kv = torch.Tensor([[11,12,12,12,12,12]]).repeat(5, 1).to(device).long()
