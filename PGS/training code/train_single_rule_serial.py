...
import make_generate_attribute as make_data
...
#model.load_state_dict(torch.load('/home/user/ybm/advance/model_attr_model_plus_type_120000_Datasets_distribute_nine_type_now.pt', map_location = 'cpu'))
...
while epoch < num_epoch:
    ...
    with tqdm(total=len(train_loader)  + len(val_loader)) as pbar:
        ...
        for _, x_train, label in train_loader:
            ...
            kv = (torch.Tensor([[11] + [12]*num_entity]).repeat(x_train.shape[0], 1)).to(device).long()
            ...
            for i in range(num_entity):
                ...
                out_train = model(x_train, kv.data) #输出
                
                kv[:, i + 1] = x_train[:, -1, i].data

                loss_out = (model.module.loss_function(x_train[:, -1, i].data, *out_train, rule_label = label, i = i) if isinstance(model, nn.DataParallel) 
                        else model.loss_function(x_train[:, -1, i], *out_train, rule_label = label, i = i))
        ...
        with torch.no_grad():
            ...
            for index, (_, x_test, label)in enumerate(val_loader):
                ...
                kv = torch.Tensor([[11] + [12]*num_entity]).repeat(x_test.shape[0], 1).to(device).long()
                
                kv.requires_grad = False
                
                for i in range(num_entity):
                
                    out_test = model(x_test, kv.data)
                    
                    kv[:, i + 1] = x_test[:, -1, i].data
    
                    loss_out = (model.module.loss_function(x_test[:, -1, i].data, *out_test, rule_label = label, i = i) if isinstance(model, nn.DataParallel) 
                            else model.loss_function(x_test[:, -1, i], *out_test, rule_label = label, i = i))
    ...
    print("epoch:{}\n loss_train: all: {:.4f}\t  rule:  {:.4f}\t  regression {:.4f}\t  choose_accuracy:  {:.4f}\t  rule_accuracy: {:.4f}\n loss_test:  all: {:.4f}\t  rule:  {:.4f}\t  regression {:.4f}\t  choose_accuracy:  {:.4f}\t  rule_accuracy: {:.4f}\n  learning_rate:{:.8f}\n".\
          format(epoch, loss_train[0], loss_train[1],  loss_train[2], accuracy[0], accuracy[1], loss_test_all[0], loss_test_all[1],  loss_test_all[2],  accuracy_val[0], accuracy_val[1], lr_decay.get_lr()[0]))
