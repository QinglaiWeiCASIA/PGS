...
if make_data.dataset == 'left_center_single_right_center_single':
    ...
    from attr_regress_with_rule import attr_regress_decoder_mask_lr as model_vic
    
    
elif make_data.dataset == 'up_center_single_down_center_single':
    
    
    from attr_regress_with_rule import attr_regress_decoder_mask_ud as model_vic
    
elif make_data.dataset == 'in_distribute_four_out_center_single':
    
    
    from attr_regress_with_rule import attr_regress_decoder_mask_oig as model_vic
    
    
elif make_data.dataset == 'in_center_single_out_center_single':
    
    
    from attr_regress_with_rule import attr_regress_decoder_mask_oic as model_vic
    ...
    with tqdm(total=len(train_loader)  + len(val_loader)) as pbar:
        ...
        for _, x_train, label_out, label_in in train_loader:
            ...
            for i in range(num_entity):
                ...
                loss_out = (model.module.loss_function(x_train[:, -1, i].data, *out_train, rule_label_out = label_out, rule_label_in = label_in, i = i) if isinstance(model, nn.DataParallel) 
                        else model.loss_function(x_train[:, -1, i], *out_train, rule_label_out = label_out, rule_label_in = label_in, i = i))
        ...
        with torch.no_grad():
            ...
            for index, (_, x_test, label_out, label_in)in enumerate(val_loader):
                ...
                for i in range(num_entity):
                    ...
                    loss_out = (model.module.loss_function(x_test[:, -1, i].data, *out_test, rule_label_out = label_out, rule_label_in = label_in, i = i) if isinstance(model, nn.DataParallel) 
                            else model.loss_function(x_test[:, -1, i], *out_test, rule_label_out = label_out, rule_label_in = label_in, i = i))
